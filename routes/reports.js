...
router.get('/reports', function (req, res) {
  ...
  const counts = {
    swimmers: db.prepare('SELECT COUNT(*) c FROM swimmers').get().c,
    active: db.prepare("SELECT COUNT(*) c FROM swimmers WHERE status='نشط'").get().c,
    subsActive: db.prepare("SELECT COUNT(*) c FROM subscriptions WHERE status='نشط'").get().c,
    subsDue: db.prepare("SELECT COUNT(*) c FROM subscriptions WHERE remaining > 0").get().c,
    sessions: db.prepare('SELECT COUNT(*) c FROM sessions').get().c,
    upcomingComps: db.prepare("SELECT COUNT(*) c FROM competitions WHERE status IN ('قادمة','جارية')").get().c
  };
...
/*                      تقرير مالي شامل                           */
/* ============================================================== */
router.get('/reports/financial', function (req, res) {
  if (!canView(req.currentUser, 'reports')) return res.status(403).render('errors/403', { layout: false, user: req.currentUser });
  const from = req.query.from || daysAgo(30);
  const to = req.query.to || today();
  const revRows = db.prepare('SELECT category, SUM(amount) total FROM revenues WHERE date BETWEEN ? AND ? GROUP BY category').all(from, to);
  const expRows = db.prepare('SELECT category, SUM(amount) total FROM expenses WHERE date BETWEEN ? AND ? GROUP BY category').all(from, to);
...
/* ============================================================== */
router.get('/reports/attendance', function (req, res) {
  if (!canView(req.currentUser, 'reports')) return res.status(403).render('errors/403', { layout: false, user: req.currentUser });
  const from = req.query.from || daysAgo(30);
  const to = req.query.to || today();
  const rows = db.prepare(`SELECT s.full_name, s.membership_no,
...
/* ============================================================== */
router.get('/reports/attendance/daily', function (req, res) {
  if (!canView(req.currentUser, 'reports')) return res.status(403).render('errors/403', { layout: false, user: req.currentUser });
  const date = req.query.date || today();
  const groupRows = db.prepare(`SELECT g.*, c.full_name AS coach_name FROM groups g
  ...
  const groups = groupRows.map(function (g) {
    ...
    let members = [];
    if (session) {
      members = db.prepare(`SELECT s.full_name, s.membership_no, l.name AS level_name, a.status, a.reason
...
/*                     تقرير الاشتراكات                           */
/* ============================================================== */
router.get('/reports/subscriptions', function (req, res) {
  if (!canView(req.currentUser, 'reports')) return res.status(403).render('errors/403', { layout: false, user: req.currentUser });
...
/*                  تقرير تقدم السباحين                            */
/* ============================================================== */
router.get('/reports/progress', function (req, res) {
  if (!canView(req.currentUser, 'reports')) return res.status(403).render('errors/403', { layout: false, user: req.currentUser });
  const rows = db.prepare(`SELECT s.full_name, s.membership_no, l.name AS level_name,
