...
router.get('/', function (req, res) {
  const data = siteData();
  const programs = db.prepare("SELECT * FROM programs WHERE status IN ('متاح','مكتمل العدد') ORDER BY id LIMIT 6").all();
  const announcements = db.prepare('SELECT * FROM announcements WHERE is_public = 1 ORDER BY id DESC LIMIT 4').all();
  const coaches = db.prepare("SELECT * FROM coaches WHERE status='active' ORDER BY id LIMIT 4").all();
  const pools = db.prepare('SELECT p.*, b.name AS branch_name FROM pools p LEFT JOIN branches b ON b.id = p.branch_id').all();
  res.render('site/index', { data, programs, announcements, coaches, pools, money, fmtDate, layout: false });
});

router.get('/programs', function (req, res) {
  const data = siteData();
  const programs = db.prepare("SELECT * FROM programs WHERE status IN ('متاح','مكتمل العدد') ORDER BY id").all();
  res.render('site/programs', { data, programs, money, layout: false });
});

router.get('/programs/:id', function (req, res) {
  const data = siteData();
  const p = db.prepare('SELECT * FROM programs WHERE id = ?').get(Number(req.params.id));
...
router.get('/coaches', function (req, res) {
  const data = siteData();
  const coaches = db.prepare("SELECT * FROM coaches WHERE status='active' ORDER BY id").all();
  res.render('site/coaches', { data, coaches, fmtDate, layout: false });
});

router.get('/announcements', function (req, res) {
  const data = siteData();
  const announcements = db.prepare('SELECT * FROM announcements WHERE is_public = 1 ORDER BY id DESC').all();
...
router.post('/contact', function (req, res) {
  ...
  const b = req.body;
  if (b.name && b.message) {
    db.prepare('INSERT INTO messages (from_user_id, to_user_id, subject, body) VALUES (?,?,?,?)').run(null, 1, 'رسالة من الموقع: ' + (b.name || ''), (b.phone || '') + ' | ' + (b.email || '') + ' | ' + b.message);
  }
