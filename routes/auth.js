...
router.post('/profile/password', function (req, res) {
  ...
  if (!verifyPassword(current || '', user.password_hash)) {
    ...
    req.session.flash = { type: 'success', message: 'تم تغيير كلمة المرور بنجاح' };
...
router.get('/search', function (req, res) {
  ...
  if (q) {
    const like = '%' + q + '%';
    results.swimmers = db.prepare(`SELECT s.*, g.full_name AS guardian_name, l.name AS level_name FROM swimmers s
    ...
    results.guardians = db.prepare('SELECT * FROM guardians WHERE full_name LIKE ? OR phone LIKE ? LIMIT 8').all(like, like);
    results.coaches = db.prepare('SELECT * FROM coaches WHERE full_name LIKE ? OR phone LIKE ? LIMIT 8').all(like, like);
    results.programs = db.prepare('SELECT * FROM programs WHERE name LIKE ? LIMIT 8').all(like);
    results.groups = db.prepare('SELECT * FROM groups WHERE name LIKE ? LIMIT 8').all(like);
  }
