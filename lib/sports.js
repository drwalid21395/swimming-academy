...
async function listActiveSports() {
  return cached('active-sports', () => db.prepare('SELECT * FROM sports WHERE is_active = 1 ORDER BY sort_order, id').all(), 30000);
}
...
async function enabledSportsForAcademy(academyId) {
  return cached('academy-sports:' + Number(academyId), async () => {
...
/* تفعيل/تعطيل لعبة لأكاديمية (ينشئ الرابط إن لم يوجد).
   التفعيل ينشط اللعبة عالمياً أيضاً (sports.is_active=1) حتى تظهر فوراً
   في التبويبات والموقع، وإلا تبقى اللعبة نشطة للأكاديمية لكنها لا تظهر
   لأن كل القوائم تشترط is_active=1. */
async function setSportEnabled(academyId, sportId, enabled) {
  ...
  if (enabled) {
    await db.prepare('UPDATE sports SET is_active = 1, sort_order = COALESCE(sort_order, 99) WHERE id = ?').run(si);
  }
  invalidate('academy-sports:' + ai);
  invalidate('active-sports');
  return true;
}
