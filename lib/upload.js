...
/* وسيط: يستقبل ملفاً واحداً من حقل + عدة ملفات من حقل آخر في نفس الطلب
   (مطلوب لأن multer يستهلك الستريم مرة واحدة فقط)
   req.file = ملف الحقل الأول (إن وُجد) — req.__images = مصفوفة روابط الحقل الثاني */
function uploadSinglePlusArray(singleField, arrayField, maxCount = 12) {
  return function (req, res, next) {
    upload.fields([
      { name: singleField, maxCount: 1 },
      { name: arrayField, maxCount: maxCount }
    ])(req, res, function (err) {
      if (err) return next(err);
      const saves = [];
      req.__images = [];
      const single = req.files && req.files[singleField] ? req.files[singleField][0] : null;
      if (single) {
        if (!single.filename) {
          single.filename = Date.now() + '-' + Math.round(Math.random() * 1e9) + path.extname(single.originalname || '').slice(0, 10);
        }
        saves.push(saveBlobToDb(single).then(function () { req.file = single; }));
      }
      const arr = (req.files && req.files[arrayField]) || [];
      for (const file of arr) {
        if (!file.filename) {
          file.filename = Date.now() + '-' + Math.round(Math.random() * 1e9) + path.extname(file.originalname || '').slice(0, 10);
        }
        saves.push(saveBlobToDb(file).then(function () { req.__images.push('/uploads/' + file.filename); }));
      }
      Promise.all(saves).then(() => next(), next);
    });
  };
}
...
module.exports = { upload, uploadAndStore, uploadFields, uploadSinglePlusArray, UPLOAD_DIR, removeUploaded, getStoredFile };
