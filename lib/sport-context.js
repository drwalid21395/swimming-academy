...
/* المجموعات (عبر رياضتها المباشرة، مع الرجوع لرياضة برنامجها للمجموعات القديمة) */
function groupClause(s, alias) {
  const id = sid(s);
  if (!id) return '';
  const a = alias ? alias + '.' : '';
  return ` AND (${a}sport_id = ${id} OR (${a}sport_id IS NULL AND ${a}program_id IN (SELECT id FROM programs WHERE deleted_at IS NULL AND sport_id = ${id})))`;
...
/* الحصص (عبر مجموعتها ← رياضتها/برنامجها) — alias = اسم جدول sessions في الاستعلام أو '' */
function sessionClause(s, alias) {
  const id = sid(s);
  if (!id) return '';
  const a = alias ? alias + '.' : '';
  return ` AND ${a}id IN (SELECT s2.id FROM sessions s2 JOIN groups g ON g.id = s2.group_id WHERE (g.sport_id = ${id} OR (g.sport_id IS NULL AND g.program_id IN (SELECT id FROM programs WHERE deleted_at IS NULL AND sport_id = ${id}))))`;
}
