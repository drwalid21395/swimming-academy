...
function defaultPermissions(all) {
  const p = {};
  for (const m of MODULES) p[m] = all ? { view: 1, add: 1, edit: 1, del: 1 } : { view: 0, add: 0, edit: 0, del: 0 };
  return p;
}
...
function can(user, module, action) {
  ...
  const p = perms[module] || { view: 0, add: 0, edit: 0, del: 0 };
  return !!(p && p[action]);
}
...
function requirePerm(module, action) {
  return (req, res, next) => {
    if (req.session.user && can(req.session.user, module, action)) return next();
    if (req.session.user) return res.status(403).render('errors/403', { layout: false, user: req.session.user });
...
module.exports = {
  ...
  can, canView, canAdd, canEdit, canDel, requirePerm,
  parseJSON
};
