...
/* إرسال إشعار نصي عام لمدير النظام (للطلبات: تغيير كلمة المرور وغيره).
   يستخدم Cloud API إن كان مفعلاً، وإلا يفتح رابط wa.me احتياطياً. */
async function notifyAdmin({ text, forAdmin = true }) {
  const s = await settingsMap();
  const token = (s.whatsapp_api_token || '').trim();
  const phoneId = (s.whatsapp_phone_id || '').trim();
  const to = (s.whatsapp_admin_notify || s.whatsapp || '').trim();
  if (!to) return { ok: false, mode: 'none', error: 'لا يوجد رقم إشعارات للمدير' };
  if (token && phoneId) {
    const r = await sendViaApi({ phone: to, text, token, phoneId });
    return { ok: true, mode: r.ok ? 'api' : 'link', url: r.ok ? '' : waLink(to, text), apiError: r.error };
  }
  return { ok: true, mode: 'link', url: waLink(to, text) };
}

module.exports = { normalizePhone, waLink, buildRenewalMessage, sendViaApi, sendReminder, logMessage, maybeSendExpiryReminders, notifyAdmin };