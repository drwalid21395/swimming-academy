...
/* إزالة الألعاب غير السباحة تماماً (بيانات تجريبية سابقة) + صفوف مهملة لأكاديميات محذوفة */
async function removeExtraSports() {
  try {
    const depTables = ['coaches', 'programs', 'announcements', 'levels', 'revenues', 'expenses', 'teams', 'competitions'];
    let used = 0;
    for (const t of depTables) {
      try {
        const r = await db.prepare(`SELECT COUNT(*) c FROM ${t} WHERE ${t}.sport_id IS NOT NULL AND ${t}.sport_id != 1`).get();
        used += (r.c || 0);
      } catch (e) { /* الجدول/العمود قد لا يكون موجوداً */ }
    }
    try {
      await db.prepare('DELETE FROM academy_sports WHERE academy_id NOT IN (SELECT id FROM academies)').run();
      await db.prepare('DELETE FROM academy_sports WHERE sport_id != 1').run();
    } catch (e) { /* قد تكون الجداول غير متاحة بعد */ }
    if (used === 0) {
      try { await db.prepare('DELETE FROM sports WHERE id != 1').run(); } catch (e) { /* تجاهل */ }
    } else {
      /* لعبة غير السباحة ما زالت تستخدمها بيانات حقيقية: لا نحذف بل نعطّلها */
      try {
        const sp = await db.all('SELECT id FROM sports WHERE id != 1');
        for (const s of sp) await db.prepare('UPDATE sports SET is_active = 0 WHERE id = ?').run(s.id);
      } catch (e) { /* تجاهل */ }
    }
  ...
  } catch (e) { console.error('تنظيف الألعاب التجريبية:', e.message); }
}
