...
async function removeTestPrograms() {
  try {
    ...
    console.log('[تنظيف] برامج اختبار موجودة:', progs.length);
    for (const p of progs) {
      let usedSw = 0, usedSub = 0, usedRes = 0;
      try { usedSw = Number((await db.prepare('SELECT COUNT(*) c FROM swimmers WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      try { usedSub = Number((await db.prepare('SELECT COUNT(*) c FROM subscriptions WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      try { usedRes = Number((await db.prepare('SELECT COUNT(*) c FROM reservations WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      const used = usedSw + usedSub + usedRes;
      console.log('[تنظيف] برنامج #' + p.id + ' مرجعيات: لاعبين=' + usedSw + ' اشتراكات=' + usedSub + ' حجوزات=' + usedRes);
      if (used) continue;
      ...
      console.log('[تنظيف] حذف برنامج #' + p.id);
    }
