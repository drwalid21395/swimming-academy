 ...
 * يعتمد على node:sqlite (DatabaseSync) المدمج في Node.js
 */
...
/* ---------- مزامنة تلقائية لأعضاء المجموعات ----------
   أي تغيير في مجموعة السباح يتم ترحيله إلى جدول swimmer_group فوراً */
try {
  db.exec(`
  CREATE TRIGGER IF NOT EXISTS sync_swimmer_group_after_insert AFTER INSERT ON swimmers
  BEGIN
    INSERT OR IGNORE INTO swimmer_group (swimmer_id, group_id)
      SELECT NEW.id, NEW.group_id WHERE NEW.group_id IS NOT NULL;
  END;

  CREATE TRIGGER IF NOT EXISTS sync_swimmer_group_after_update AFTER UPDATE OF group_id ON swimmers
  BEGIN
  ...
  console.error('خطأ في إنشاء محفزات المزامنة:', e.message);
}

/* ---------- ترحيلات للبيانات القائمة ---------- */
function dbHasColumn(table, col) {
  try {
    return db.prepare(`SELECT name FROM pragma_table_info(?) WHERE name = ?`).all(table, col).length > 0;
  } catch (e) { return false; }
}

if (!dbHasColumn('assessment_criteria', 'level_id')) {
  try { db.exec('ALTER TABLE assessment_criteria ADD COLUMN level_id INTEGER'); } catch (e) { console.error('ترحيل level_id:', e.message); }
}

if (!dbHasColumn('coaches', 'cv')) {
  try { db.exec('ALTER TABLE coaches ADD COLUMN cv TEXT'); } catch (e) { console.error('ترحيل cv:', e.message); }
}

if (!dbHasColumn('staff', 'cv')) {
  try { db.exec('ALTER TABLE staff ADD COLUMN cv TEXT'); } catch (e) { console.error('ترحيل cv للموظفين:', e.message); }
}

if (!dbHasColumn('programs', 'schedule')) {
  try { db.exec("ALTER TABLE programs ADD COLUMN schedule TEXT DEFAULT '[]'"); } catch (e) { console.error('ترحيل schedule للبرامج:', e.message); }
}

if (!dbHasColumn('tests', 'race_type')) {
  try { db.exec('ALTER TABLE tests ADD COLUMN race_type TEXT'); } catch (e) { console.error('ترحيل race_type للاختبارات:', e.message); }
}

if (!dbHasColumn('tests', 'level_id')) {
  try { db.exec('ALTER TABLE tests ADD COLUMN level_id INTEGER'); } catch (e) { console.error('ترحيل level_id للاختبارات:', e.message); }
}

/* إعدادات افتراضية (لا تُستبدل إذا كان المستخدم عدّلها لاحقاً) */
(function seedSettingsDefaults() {
  try {
    const ins = db.prepare('INSERT OR IGNORE INTO settings (key, value) VALUES (?, ?)');
    ins.run('program_types', JSON.stringify(['تعليم سباحة', 'تدريب سباحة', 'فرق', 'إنقاذ', 'سلامة في الماء', 'إعداد معلم سباحة', 'معسكر', 'دورة خاصة']));
    ins.run('test_types', JSON.stringify(['مستوى', 'زمن', 'بطولة', 'عام']));
    ins.run('home_images', '[]');
...
(function seedCurriculum() {
  try {
    const ver = (db.prepare("SELECT value FROM settings WHERE key = 'curriculum_version'").get() || {}).value;
    if (ver === 'v2') return;

    let levels = db.prepare('SELECT * FROM levels ORDER BY order_no').all();
    if (!levels.length) {
      const insL = db.prepare('INSERT INTO levels (name, order_no) VALUES (?,?)');
      [...CURRICULUM_LEVELS, 'فريق أكاديمية', 'فريق بطولات'].forEach((n, i) => insL.run(n, i + 1));
      levels = db.prepare('SELECT * FROM levels ORDER BY order_no').all();
    }

    const updL = db.prepare('UPDATE levels SET name = ? WHERE id = ?');
    levels.slice(0, 7).forEach((lvl, i) => updL.run(CURRICULUM_LEVELS[i], lvl.id));

    db.prepare('DELETE FROM assessment_criteria').run();
    const insC = db.prepare('INSERT INTO assessment_criteria (name, category, program_type, order_no, level_id) VALUES (?,?,?,?,?)');
    CURRICULUM.forEach((items, idx) => {
      const levelId = levels[idx].id;
      items.forEach((name, i) => insC.run(name, CURRICULUM_LEVELS[idx], 'all', i + 1, levelId));
    });
    db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('curriculum_version','v2')").run();
    console.log('تم تطبيق المنهج التعليمي الجديد (المستويات السبع)');
  } catch (e) { console.error('خطأ في تطبيق المنهج:', e.message); }
})();

/* ---------- المعايير العامة: مشتركة تُضمّن تلقائياً في كل المستويات ---------- */
(function seedGeneralCriteria() {
  try {
    if ((db.prepare("SELECT value FROM settings WHERE key = 'general_criteria_v1'").get() || {}).value === '1') return;
    const sharedNames = ['السرعة', 'الالتزام', 'الانضباط'];
    sharedNames.forEach(function (n) {
      db.prepare('DELETE FROM assessment_criteria WHERE level_id IS NOT NULL AND name = ?').run(n);
    });
    const existing = db.prepare('SELECT name FROM assessment_criteria WHERE level_id IS NULL').all().map(r => r.name);
    let order = db.prepare('SELECT COALESCE(MAX(order_no),0)+1 n FROM assessment_criteria WHERE level_id IS NULL').get().n;
    sharedNames.forEach(function (n) {
      if (existing.indexOf(n) < 0) {
        db.prepare('INSERT INTO assessment_criteria (name, category, program_type, order_no, level_id) VALUES (?,?,?,?,NULL)')
          .run(n, 'معايير عامة', 'all', order++);
      }
    });
    db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('general_criteria_v1','1')").run();
