...
async function removeTestPrograms() {
  try {
    ...
    for (const p of progs) {
      let used = 0;
      try { used += Number((await db.prepare('SELECT COUNT(*) c FROM swimmers WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      try { used += Number((await db.prepare('SELECT COUNT(*) c FROM subscriptions WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      try { used += Number((await db.prepare('SELECT COUNT(*) c FROM reservations WHERE program_id = ?').get(p.id) || {}).c || 0); } catch (e) {}
      if (used) continue;
