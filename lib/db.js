...
async function initDatabase() {
  ...
  /* إسناد بيانات ما قبل «الألعاب المتعددة» إلى لعبة السباحة (1) — كل ما هو موجود
     حالياً (مستويات/إيرادات/مصروفات) أنشئ في فترة كانت الأكاديمية سباحة فقط. */
  try {
...
async function seedSports() {
  try {
    const defs = [
      { name: 'سباحة', icon: 'fa-person-swimming', description: 'تعليم وتدريب السباحة لجميع المستويات والأعمار.' },
      { name: 'كرة القدم', icon: 'fa-futbol', description: 'تدريب كرة القدم للمراحل السنية المختلفة.' },
      { name: 'الكاراتيه', icon: 'fa-hand-fist', description: 'تدريب فنون الدفاع عن النفس والكاراتيه.' },
      { name: 'الجيم واللياقة', icon: 'fa-dumbbell', description: 'برامج اللياقة البدنية وتقوية الجسم.' },
      { name: 'تنس الطاولة', icon: 'fa-table-tennis-paddle-ball', description: 'تدريب تنس الطاولة للمبتدئين والمحترفين.' }
    ];
    const cnt = (await db.prepare('SELECT COUNT(*) c FROM sports').get() || { c: 0 });
    if (!cnt.c) {
      const ins = db.prepare('INSERT INTO sports (name, icon, description, sort_order) VALUES (?,?,?,?)');
      for (let i = 0; i < defs.length; i++) {
        await ins.run(defs[i].name, defs[i].icon, defs[i].description, i + 1);
      }
    }
    /* تفعيل الألعاب النشطة لكل الأكاديميات (INSERT OR IGNORE لا يمس التعديلات) */
    const active = await db.all('SELECT id FROM sports WHERE is_active = 1');
    const academies = await db.all('SELECT id FROM academies');
    if (active.length && academies.length) {
      const ins2 = db.prepare('INSERT OR IGNORE INTO academy_sports (academy_id, sport_id, is_enabled) VALUES (?,?,1)');
      for (const a of academies) for (const s of active) await ins2.run(a.id, s.id);
    }
