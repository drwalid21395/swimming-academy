...
const { measure, finish } = require('./perf');

const DB_URL = process.env.DB_URL || (process.env.DB_PATH ? (String(process.env.DB_PATH).startsWith('file:') ? process.env.DB_PATH : 'file:' + process.env.DB_PATH) : ('file:' + path.join(__dirname, '..', 'data.db')));
...
const db = {
  ...
  prepare(sql) {
    ...
    const runStmt = (args) => {
      const timing = measure(sql);
      return client.execute({ sql, args: sanitizeArgs(args) }).finally(() => finish(timing));
    };
    return {
  ...
  batch(stmts) {
    const tenant = getTenant();
    return client.batch(stmts.map(s => {
      const stmt = typeof s === 'string' ? { sql: s, args: [] } : s;
      let sql = stmt.sql;
      if (tenant && tenant.academyId) sql = scopeSql(sql, tenant.academyId);
      return { sql, args: sanitizeArgs(stmt.args) };
    }), 'write');
  },
  /* معاملة صريحة فوق نفس الـ client مع تطبيق العزل.
     لا نستخدم BEGIN/COMMIT عبر execute منفصلة لأنها تفشل على الاتصالات البعيدة
     (Turso/HTTP) حيث لا تُشارك الجمل نفس الجلسة. */
  async transaction(mode = 'write') {
    ...
    const exec = (sql, args) => {
      ...
      if (tenant && tenant.academyId) sql = scopeSql(sql, tenant.academyId);
      const timing = measure(sql);
      return tx.execute({ sql, args: sanitizeArgs(args) }).finally(() => finish(timing));
    };
    return {
      execute: exec,
      async get(sql, ...args) { const rs = await exec(sql, args); return rs.rows.length ? rs.rows[0] : undefined; },
      async all(sql, ...args) { const rs = await exec(sql, args); return Array.from(rs.rows); },
      async run(sql, ...args) {
...
async function initDatabase() {
  /* قاعدة Vercel/Turso مهيأة مسبقاً؛ إعادة المخطط والترحيلات في كل Cold Start
     تضيف 10+ ثوانٍ قبل أول رد. استخدم DB_BOOTSTRAP=1 عند تهيئة قاعدة جديدة. */
  if (process.env.VERCEL && IS_REMOTE && process.env.DB_BOOTSTRAP !== '1') return;
  ...
  const migrations = [
    ...
    "ALTER TABLE reservations ADD COLUMN swimmer_id INTEGER",
    "ALTER TABLE reservations ADD COLUMN guardian_id INTEGER",
    "ALTER TABLE reservations ADD COLUMN converted_at TEXT",
    "ALTER TABLE reservations ADD COLUMN sport_id INTEGER",
    "ALTER TABLE reservations ADD COLUMN guardian_name TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_relation TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_whatsapp TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_email TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_address TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_national_id TEXT",
    "ALTER TABLE reservations ADD COLUMN guardian_notes TEXT",
    "ALTER TABLE subscriptions ADD COLUMN subscription_type TEXT DEFAULT 'normal'",
    "ALTER TABLE subscriptions ADD COLUMN price_per_session REAL DEFAULT 0",
    "ALTER TABLE subscriptions ADD COLUMN sport_id INTEGER",
    'ALTER TABLE programs ADD COLUMN sport_id INTEGER',
    'ALTER TABLE coaches ADD COLUMN sport_id INTEGER',
    'ALTER TABLE announcements ADD COLUMN sport_id INTEGER',
    'ALTER TABLE levels ADD COLUMN sport_id INTEGER',
    'ALTER TABLE revenues ADD COLUMN sport_id INTEGER',
    'ALTER TABLE expenses ADD COLUMN sport_id INTEGER',
    'ALTER TABLE teams ADD COLUMN sport_id INTEGER',
    'ALTER TABLE competitions ADD COLUMN sport_id INTEGER',
    'ALTER TABLE swimmers ADD COLUMN sport_id INTEGER',
    'ALTER TABLE staff ADD COLUMN sport_id INTEGER',
    'ALTER TABLE groups ADD COLUMN sport_id INTEGER'
  ];
  for (const m of migrations) {
    try { await client.execute(m); } catch (e) { /* العمود موجود مسبقاً */ }
  }
  /* الفهارس تحسين اختياري؛ لا يجب أن تمنع إقلاع الموقع إذا رفضتها قاعدة بعيدة أو قديمة. */
  if (!IS_REMOTE) try {
    await client.executeMultiple(`
      CREATE INDEX IF NOT EXISTS idx_users_academy_status ON users(academy_id, status);
      CREATE INDEX IF NOT EXISTS idx_swimmers_academy_sport ON swimmers(academy_id, sport_id, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_swimmers_program ON swimmers(program_id, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_programs_academy_sport ON programs(academy_id, sport_id, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_groups_academy_sport ON groups(academy_id, sport_id, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_groups_program ON groups(program_id, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_sessions_group_date ON sessions(group_id, date, deleted_at);
      CREATE INDEX IF NOT EXISTS idx_attendance_session_swimmer ON attendance(session_id, swimmer_id);
      CREATE INDEX IF NOT EXISTS idx_subscriptions_academy_sport ON subscriptions(academy_id, sport_id, status, created_at);
      CREATE INDEX IF NOT EXISTS idx_subscriptions_swimmer_status ON subscriptions(swimmer_id, status, id);
      CREATE INDEX IF NOT EXISTS idx_payments_subscription ON payments(subscription_id, paid_date);
      CREATE INDEX IF NOT EXISTS idx_notifications_recipients_user_read ON notification_recipients(user_id, is_read);
    `);
  } catch (e) { console.error('فشل إنشاء فهارس الأداء، يستمر التشغيل:', e.message); }

  /* إسناد بيانات ما قبل «الألعاب المتعددة» إلى لعبة السباحة (1) — كل ما هو موجود
     حالياً (برامج/كباتن/إعلانات/مستويات/إيرادات/مصروفات) أنشئ في فترة كانت الأكاديمية
     سباحة فقط، فيرتجع للعبة السباحة وحدها ولا يُشارَك بين الألعاب. */
  try {
    await db.prepare('UPDATE programs SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE coaches SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE announcements SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE levels SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE revenues SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE expenses SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE teams SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE competitions SET sport_id = 1 WHERE sport_id IS NULL').run();
    await db.prepare('UPDATE staff SET sport_id = 1 WHERE sport_id IS NULL').run();
    /* المجموعات القديمة: ترجع لرياضة برنامجها، وإلا للسباحة (لا تُشترَك بين الألعاب) */
    await db.prepare('UPDATE groups SET sport_id = COALESCE((SELECT p.sport_id FROM programs p WHERE p.id = groups.program_id), 1) WHERE sport_id IS NULL').run();
  } catch (e) { /* الأعمدة غير متاحة بعد — تُهمل */ }
  ...
  await seedSports();
  await removeExtraSports();
  await removeTestPrograms();
  await renamePlayerTermsInDb();
...
-- سجل نقل اللاعبين بين المجموعات/المدرّبين
...
-- =============================== الألعاب الرياضية ===============================
-- كتالوج الألعاب (مشترك على مستوى المنصة: سباحة، كرة قدم، كاراتيه، جيم ...)
CREATE TABLE IF NOT EXISTS sports (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  icon TEXT DEFAULT 'fa-medal',                -- أيقونة FontAwesome
  description TEXT,
  is_active INTEGER DEFAULT 1,                 -- 1 = تظهر في كتالوج المنصة
  sort_order INTEGER DEFAULT 1,
  created_at TEXT DEFAULT (datetime('now','localtime'))
);

-- تفعيل الألعاب لكل أكاديمية + محتوى موقع اللعبة الخاص بها
CREATE TABLE IF NOT EXISTS academy_sports (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  academy_id INTEGER NOT NULL,
  sport_id INTEGER NOT NULL,
  is_enabled INTEGER DEFAULT 1,                -- تفعيل اللعبة لهذه الأكاديمية
  website TEXT DEFAULT '{}',                   -- JSON: { about, banner, gallery:[] }
  created_at TEXT DEFAULT (datetime('now','localtime')),
...
async function repairSystemRoles() {
  const MODS = ['dashboard','swimmers','guardians','coaches','staff','programs','levels','groups','sessions','attendance','assessments','tests','teams','competitions','subscriptions','payments','revenues','expenses','coachPayments','incoming','outgoing','documents','notifications','complaints','reservations','reports','branches','pools','users','settings','auditLog','site','trainerAttendance','staffAttendance','payroll','sports'];
...
/* ---------- بذر الألعاب الرياضية ----------
   ينشئ كتالوج الألعاب الافتراضي (مرة واحدة) ثم يُفعّل الألعاب النشطة
   تلقائياً لكل أكاديمية (يمكن إيقاف الألعاب من شاشة التحكم المركزية). */
async function seedSports() {
  try {
    /* لعبة واحدة فقط: السباحة. الألعاب الأخرى تُزال (بيانات تجريبية) ويمكن
       إضافتها لاحقاً يدوياً من منصة الإدارة. */
    const cnt = (await db.prepare('SELECT COUNT(*) c FROM sports WHERE name = \'سباحة\'').get() || { c: 0 });
    if (!cnt.c) {
      await db.prepare('INSERT INTO sports (name, icon, description, sort_order) VALUES (?,?,?,?)')
        .run('سباحة', 'fa-person-swimming', 'تعليم وتدريب السباحة لجميع المستويات والأعمار.', 1);
    }
  } catch (e) { console.error('تهيئة الألعاب الرياضية:', e.message); }
}

/* تنظيف مراجع مهملة فقط — لا نحذف أي لعبة أنشأتها الأكاديمية ولا نعطّلها
   (العمل السابق كان يحذف/يعطّل الألعاب غير السباحة عند كل إقلاع، فأي لعبة
   جديدة مثل «كرة القدم» كانت تُمحى عند أول إقلاب/برود على Vercel مع بياناتها. */
async function removeExtraSports() {
  try {
    await db.prepare('DELETE FROM academy_sports WHERE academy_id NOT IN (SELECT id FROM academies)').run();
  } catch (e) { /* قد تكون الجداول غير متاحة بعد */ }
  /* جداول مؤقتة أنشأت في مرحلة تطوير سابقة */
  try { await client.execute('DROP TABLE IF EXISTS _deltest'); } catch (e) { /* تجاهل */ }
  try { await client.execute('DROP TABLE IF EXISTS scratch_test'); } catch (e) { /* تجاهل */ }
}

/* إزالة البرامج والمجموعات التجريبية («اختبار» في الاسم) — فقط لو لم تُستخدم:
   لا لاعبين مرتبطين ولا اشتراكات ولا حجوزات ولا حصص/حضور. */
async function deleteUnusedGroup(gid) {
  try {
    const used = Number((await db.prepare('SELECT COUNT(*) c FROM swimmer_group WHERE group_id = ?').get(gid) || {}).c || 0)
      + Number((await db.prepare('SELECT COUNT(*) c FROM sessions WHERE group_id = ?').get(gid) || {}).c || 0)
      + Number((await db.prepare('SELECT COUNT(*) c FROM subscriptions WHERE group_id = ?').get(gid) || {}).c || 0);
    if (used) return;
    const sess = await db.all('SELECT id FROM sessions WHERE group_id = ?', gid);
    for (const s of sess) { try { await db.prepare('DELETE FROM attendance WHERE session_id = ?').run(s.id); } catch (e) {} }
    try { await db.prepare('DELETE FROM sessions WHERE group_id = ?').run(gid); } catch (e) {}
    try { await db.prepare('DELETE FROM swimmer_group WHERE group_id = ?').run(gid); } catch (e) {}
    try { await db.prepare('DELETE FROM groups WHERE id = ?').run(gid); } catch (e) {}
  } catch (e) { /* تجاهل */ }
}
async function removeTestPrograms() {
  try {
    const progs = await db.all("SELECT id FROM programs WHERE name LIKE '%اختبار%'");
    for (const p of progs) {
      /* فكّ ارتباط أي لاعب/اشتراك/حجز بهذا البرنامج ثم حذف البرنامج ومجموعاته */
      try { await db.prepare('UPDATE swimmers SET program_id = NULL WHERE program_id = ?').run(p.id); } catch (e) {}
      try { await db.prepare('UPDATE subscriptions SET program_id = NULL WHERE program_id = ?').run(p.id); } catch (e) {}
      try { await db.prepare('UPDATE reservations SET program_id = NULL WHERE program_id = ?').run(p.id); } catch (e) {}
      const gr = await db.all('SELECT id FROM groups WHERE program_id = ?', p.id);
      for (const g of gr) await deleteUnusedGroup(g.id);
      try { await db.prepare('DELETE FROM programs WHERE id = ?').run(p.id); } catch (e) {}
    }
    const grps = await db.all("SELECT id FROM groups WHERE name LIKE '%اختبار%'");
    for (const g of grps) await deleteUnusedGroup(g.id);
  } catch (e) { console.error('تنظيف برامج الاختبار:', e.message); }
}

/* توحيد المصطلح في النصوص المخزنة سابقاً: سباح → لاعب (مع حماية كلمة «السباحة»/«سباحة») */
async function renamePlayerTermsInDb() {
  const FIELDS = [
    ['reservations', ['status', 'notes']],
    ['announcements', ['title', 'body']],
    ['messages', ['subject', 'body']],
    ['competitions', ['name', 'note']],
    ['swimmers', ['notes']],
    ['whatsapp_messages', ['swimmer_name', 'message']],
  ...
  const fn = (s) => {
    let t = String(s || '');
    t = t.replace(/السباحون/g, 'اللاعبون');
    t = t.replace(/السباحين/g, 'اللاعبين');
    t = t.replace(/سباحين/g, 'لاعبين');
    t = t.replace(/السباح أو اللاعب/g, 'اللاعب');
    t = t.replace(/السباح(?!ة|ات)/g, 'اللاعب');
    t = t.replace(/(?<!ال)سباح(?!ة|ات)/g, 'لاعب');
    return t;
  };
  for (const [table, cols] of FIELDS) {
    try {
      const colsList = cols.join(',');
      const rows = await db.all(`SELECT id, ${colsList} FROM ${table} WHERE ${cols.map(c => `${c} LIKE '%سباح%'`).join(' OR ')}`);
      for (const row of rows) {
        const sets = [];
        const vals = [];
        for (const c of cols) {
          const nv = fn(row[c]);
          if (nv === row[c]) continue;
          sets.push(`${c} = ?`);
          vals.push(nv);
        }
        if (!sets.length) continue;
        vals.push(row.id);
        await db.prepare(`UPDATE ${table} SET ${sets.join(', ')} WHERE id = ?`).run(...vals);
      }
    } catch (e) { /* الجدول غير متاح */ }
  }
}
...
async function seedAcademyBaseline(academyId) {
  ...
  try {
    ...
    const insL = db.prepare('INSERT INTO levels (name, order_no, academy_id, sport_id) VALUES (?,?,?,?)');
    const inserted = [];
    for (let i = 0; i < names.length; i++) {
      const r = await insL.run(names[i], i + 1, ai, 1);
  ...
  /* تفعيل الألعاب النشطة تلقائياً للأكاديمية الجديدة */
  try {
    const activeSp = await db.all('SELECT id FROM sports WHERE is_active = 1');
    const insSp = db.prepare('INSERT OR IGNORE INTO academy_sports (academy_id, sport_id, is_enabled) VALUES (?,?,1)');
    for (const s of activeSp) await insSp.run(ai, s.id);
  } catch (e) { console.error('خطأ في بذر ألعاب الأكاديمية #' + ai + ':', e.message); }
}
