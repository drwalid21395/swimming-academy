...
const db = {
  ...
  async transaction(mode = 'write') {
    ...
    const exec = (sql, args) => {
      const tenant = getTenant();
      if (tenant && tenant.academyId) sql = scopeSql(sql, tenant.academyId);
      return tx.execute({ sql, args: sanitizeArgs(args) });
    };
    return {
      execute: exec,
...
async function initDatabase() {
  /* قاعدة Turso على Vercel مهيأة مسبقاً. إعادة تنفيذ المخطط والترحيلات مع كل
     Cold Start تؤخر أول طلب عدة ثوانٍ؛ شغّلها يدوياً فقط عند الحاجة عبر
     DB_BOOTSTRAP=1 أثناء تهيئة قاعدة جديدة. */
