...
async function initDatabase() {
  ...
  const migrations = [
    ...
    'ALTER TABLE password_reset_requests ADD COLUMN email_status TEXT',
    'ALTER TABLE password_reset_requests ADD COLUMN wa_status TEXT',
    'ALTER TABLE password_reset_requests ADD COLUMN error TEXT'
  ];
  ...
  try {
    ...
    await ins.run('notify_email', 'waa11@fayoum.edu.eg');
    await ins.run('whatsapp_admin_notify', '01002882441');
    await ins.run('resend_api_key', '');
    await ins.run('resend_from', '');
...
-- طلبات تغيير كلمة المرور (يتم الإشعار بها للمدير فقط ولا تُغيَّر تلقائياً)
CREATE TABLE IF NOT EXISTS password_reset_requests (
  ...
  wa_status TEXT,                            -- sent | failed | link | none
  error TEXT,
  ip TEXT,
  created_at TEXT DEFAULT (datetime('now','localtime'))
);
