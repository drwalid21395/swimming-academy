...
const { effectiveSport } = require('./sport-context');
...
function buildFieldValues(fields, body) {
  ...
  for (const f of fields) {
    ...
    /* الحقول الاختيارية الفارغة (قوائم/أرقام تُربط بمفاتيح أجنبية غالباً) تُحفظ كـ NULL */
    if (f.type === 'select' && val === '') val = null;
    if (f.number) val = (val === '' || val === null || val === undefined) ? null : Number(val);
    values[f.key] = val;
  }
  return values;
}
...
module.exports = function crud(app, base, opts) {
  ...
  app.get(base, async function (req, res) {
    ...
    if (opts.beforeRender) rows = await opts.beforeRender(rows, req);
  ...
  app.post(base + '/new', uploadMiddle, async function (req, res) {
    ...
    // ربط السجل برياضة التبويب النشط (أو رياضة الأكاديمية الوحيدة)
    if (opts.sportField && values[opts.sportField] === undefined) {
      const es = effectiveSport(req);
      if (es > 0) values[opts.sportField] = es;
    }
  ...
  if (opts.view) {
    ...
    app.post(base + '/:id/edit', uploadMiddle, async function (req, res) {
      ...
      const values = buildFieldValues(await getFields(opts), req.body);
      if (opts.beforeSave) Object.assign(values, opts.beforeSave(req.body, req) || {});
      // سجلّات قديمة بلا رياضة: نملأ رياضتها عند تعديلها تحت تبويب/أكاديمية وحيدة دون مساس بالمعيّنة
      if (opts.sportField && values[opts.sportField] === undefined) {
        const es = effectiveSport(req);
        if (es > 0) {
          const cur = (await db.prepare(`SELECT ${opts.sportField} FROM ${table} WHERE id = ?`).get(id)) || {};
          if (!cur[opts.sportField] || Number(cur[opts.sportField]) === 0) values[opts.sportField] = es;
        }
      }
